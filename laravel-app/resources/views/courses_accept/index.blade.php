...
@section('content')
 ...
 <button class="btn btn-outline-primary" style="display: none" onclick="msg_decline()" id="position-top-start_decline"></button> 
...
<div class="breadcrumb-wrapper">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="#">طلبات الدورات</a>
        </li>
    </ol>
</div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel33">عرض تفاصيل الدورة </h4>
                ...
                <div class="row" style="padding:3%">
                    <div class="col-md-6">
                        <h1 id="name_course"></h1>
                    </div>
                    <div class="col-md-6">
                        <img src="" id="image2" style="width:73%" alt="">
                    </div>
                    <div class="col-md-6">
                        <h1 id="type2"></h1>
                    </div>
                    ...
                    <div class="col-md-12" id="descriptionn">
                      
                    </div>
                </div>

                <div id="showw" style="padding: 10%"></div>
...
<div class="form-modal-ex">
    <div class="modal fade text-left" id="edit_user" tabindex="-1" role="dialog" aria-labelledby="myModalLabel33" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel33">موافقة  </h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form id="edit_user_form">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                                ...
                                <input type="hidden" name="teacher_id" id="teacher_id_accept">
                                <label>سيتم الموافقة على هذه الدورة الان ! </label>
                    ...
                    <div class="modal-footer">
                        <button type="button" style="display: none" id="editing2" class="btn btn-primary btn-block"> يتم الموافقة ...</button>
                        <button type="button" id="editing" onclick="do_update()" class="btn btn-primary btn-block">موافقة</button>
                    </div>
                </form>
            </div>
        </div>
...
{{-- modal decline --}}
<div class="form-modal-ex">
    <div class="modal fade text-left" id="decline_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel33" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel33">رفض الدورة  </h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form id="decline_form">
                    ...
                    <div class="modal-body">
                        <div class="col-md-12"> 
                        <div class="row">
                                <input type="hidden" name="id" id="id22">
                                <input type="hidden" name="teacher_id" id="teacher_id_decline">
                                <div class="col-md-12">
                                    <label> سبب الرفض</label>
                                    <textarea name="decline" id="decline" class="form-control" cols="4" rows="10"></textarea>
                                </div>
                    ...
                    <div class="modal-footer">
                        <button type="button" style="display: none" id="editing223" class="btn btn-danger btn-block"> يتم الرفض ...</button>
                        <button type="button" id="editing23" onclick="do_update2()" class="btn btn-danger btn-block">رفض</button>
                    </div>
                </form>
            </div>
        </div>
...
@section('js')
    ...
    <script>
     ...
     <script type="text/javascript">
        ...
        var table = $('.yajra-datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('get_all_courses_accept') }}",
            columns: [
    ...
    <script>
        $('#modal_add').on('show.bs.modal', function(event) {
            ...
            var modal = $(this)
            modal.find('.modal-dialog #showw').html(showw);
            modal.find('.modal-dialog #type2').html(type);
            modal.find('.modal-dialog #price2').html(price);
            modal.find('.modal-dialog #name_course').text(name);
            modal.find('.modal-dialog #descriptionn').text(description);
            $(".modal-dialog #image2").attr('src', image);
...
<script>
    ...
    <script>
        $('#edit_user').on('show.bs.modal', function(event) {
            ...
            modal.find('.modal-body #teacher_id_accept').val(teacher_id);
    ...
    {{-- decline show user --}}
    <script>
        $('#decline_modal').on('show.bs.modal', function(event) {
            ...
            var modal = $(this)
            modal.find('.modal-body #id22').val(id);
            modal.find('.modal-body #teacher_id_decline').val(teacher_id);
   ...
   <script>
        function do_update2(){
            ...
            $("#editing223").css("display", "block");

            var formData = new FormData($('#decline_form')[0]);
                $.ajax({
                    type: 'post',
                    enctype: 'multipart/form-data',
                    url: "{{route('decline_this_course')}}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    cache: false,
                    success: function (data) {
                        ...
                        $("#editing223").css("display", "none");
                
                        $('.close').click();
                    
                        $('#position-top-start_decline').click();
                        $('.yajra-datatable').DataTable().ajax.reload(null, false);
