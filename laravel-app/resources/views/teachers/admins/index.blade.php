                                    ...
                                    {{-- <select class="form-select form-control" size="3" name="roles_name" id="roles_name" aria-label=".form-select-lg example"> --}}
                                        <select class="form-select form-select-lg mb-3 form-control" name="roles_name" id="roles_name" aria-label=".form-select-lg example">
                                            ...
                                            if(Auth::guard('teachers')->user()->parent == 0){
                                                $m = App\Models\Role::where('teacher_id',Auth::guard('teachers')->user()->id)->where('guard_name','teachers')->get();
                                            }else{
                                                $m = App\Models\Role::where('teacher_id',Auth::guard('teachers')->user()->parent)->where('guard_name','teachers')->get();
                                            }
                                            ...
                                            @foreach ($m as $n) 
                                               ...
                                               <?php  $name_removedNumbers = preg_replace('/\d+/', '', $n->name); ?>

                                                <option value="{{ $n->name }}">{{ $name_removedNumbers }}</option>
...
@section('js')
    ...
    <script>
     ...
     <script type="text/javascript">
        ...
        var table = $('.yajra-datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('teachers.get_all_admins') }}",
            columns: [
        ...
        $(document).on('click', '#add_user', function (e) {
            ...
            var formData = new FormData($('#add_user_form')[0]);
                $.ajax({
                    ...
                    url: "{{route('teachers.store_admins')}}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    cache: false,
        ...
        function do_update(){
            ...
            var formData = new FormData($('#edit_user_form')[0]);
                $.ajax({
                    type: 'post',
                    enctype: 'multipart/form-data',
                    url: "{{route('teachers.update_admins')}}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    cache: false,
   ...
   <script>
        function do_delete(){
            ...
            $.ajax({
                type: 'post',
                url: "{{route('teachers.destroy_admins')}}",
                data: formData,
                processData: false,
                contentType: false,
                success: function (data) {
