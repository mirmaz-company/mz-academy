...
<div class="breadcrumb-wrapper">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="index.html">   {{ trans('main_trans.main_page') }} </a>
        </li>
        <li class="breadcrumb-item"><a href="#">اللغة </a>
        </li>
    </ol>
</div>
                ...
                <table class="datatables-basic table table-responsive-md yajra-datatable">
                    <thead>
                        <tr>
                            ...
                            <th>#</th>
                            <th>الكلمة الثابتة</th>
                            <th> الجملة بالعربي</th>
                            <th> الجملة بالانجلش</th>
                            <th>العمليات</th>
...
<div class="form-modal-ex">
    <div class="modal fade text-left" id="edit_user" tabindex="-1" role="dialog" aria-labelledby="myModalLabel33" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel33">تعديل لغة </h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <label>  الكلمة بالعربي</label>
                                <div class="form-group">
                                    <input type="text" placeholder="word_ar" name="word_ar" id="word_ar2"  class="form-control" />
                                    <span id="word_ar_error" class="text-danger"></span>
                                </div>
                            </div>
                            <div class="col-md-12">
                                <label> الكلمة بالانجلش </label>
                                <div class="form-group">
                                    <input type="text" placeholder="word_en" name="word_en" id="word_en2"  class="form-control" />
                                    <span id="word_en_error" class="text-danger"></span>
                                </div>
                            </div>
    ...
    <script>
     ...
     <script type="text/javascript">
        ...
        var table = $('.yajra-datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('get_all_languages') }}",
            columns: [
                {data: 'DT_RowIndex',  name: 'DT_RowIndex', orderable: false, searchable: false},
                {data: 'fixed'         ,name: 'fixed'},
                {data: 'word_ar'         ,name: 'word_ar'},
                {data: 'word_en'         ,name: 'word_en'},
...
<script>
    ...
    <script>
        $('#edit_user').on('show.bs.modal', function(event) {
            ...
            var fixed =                button.data('fixed')
            var word_en =                button.data('word_en')
            var word_ar =                button.data('word_ar')
            
            
            var modal = $(this)
            modal.find('.modal-body #id2').val(id);
            modal.find('.modal-body #fixed2').val(fixed);
            modal.find('.modal-body #word_en2').val(word_en);
            modal.find('.modal-body #word_ar2').val(word_ar);
