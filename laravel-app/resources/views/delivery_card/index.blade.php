...
<div class="breadcrumb-wrapper">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="#">طلبات البطاقات </a>
        </li>
    </ol>
</div>
...
<div class="modal fade modal-danger text-left" id="delete_user" tabindex="-1" role="dialog" aria-labelledby="myModalLabel120" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="myModalLabel120">الغاء الطلب  </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="delete_user_form">
                    ...
                    <input type="hidden" name="user_id" id="id34">
...
@section('js')
    ...
    <script>
     ...
     <script type="text/javascript">
        ...
        var table = $('.yajra-datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('get_all_delivery_card') }}",
            columns: [
                {data: 'DT_RowIndex',        name: 'DT_RowIndex', orderable: false, searchable: false},
                {data: 'name'               ,name: 'name'},
                {data: 'mobile'             ,name: 'mobile'},
                {data: 'type_card_id'       ,name: 'type_card_id'},
                {data: 'city'               ,name: 'city'},
                {data: 'address'            ,name: 'address'},
                {data: 'near_from'          ,name: 'near_from'},
    ...
    <script>
        ...
        function accept(id,user_id){
         
               $("#accept"+id).html("جاري القبول...");
         
                $.ajax({
                    type: 'post',
                    url: "{{route('accept_price_card')}}",
                    data:{
                        _token:'{{ csrf_token() }}',
                        id: id,
                        user_id: user_id,
    ...
    <script>
        ...
        function deny(id,user_id){
               $("#deny"+id).html(" جاري الرفض...");
         
                $.ajax({
                    type: 'post',
                    url: "{{route('deny_price_card')}}",
                    data:{
                        _token:'{{ csrf_token() }}',
                        id: id,
                        user_id: user_id,
    ...
    <script>
        ...
        function delivery(id,user_id){
               $("#delivery"+id).html("جاري التحويل ...");
         
                $.ajax({
                    type: 'post',
                    url: "{{route('delivery_price_card')}}",
                    data:{
                        _token:'{{ csrf_token() }}',
                        id: id,
                        user_id: user_id,
    ...
    <script>
        ...
        function done(id,user_id){
               $("#done"+id).html("يتم الانتهاء  ...");
         
                $.ajax({
                    type: 'post',
                    url: "{{route('done_price_card')}}",
                    data:{
                        _token:'{{ csrf_token() }}',
                        id: id,
                        user_id: user_id,
   ...
   <script>
    ...
    <script>
        $('#delete_user').on('show.bs.modal', function(event) {
            ...
            modal.find('.modal-body #id34').val(user_id);
        })
    </script>
