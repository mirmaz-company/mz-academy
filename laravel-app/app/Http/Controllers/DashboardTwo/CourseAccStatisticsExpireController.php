...
use App\Exports\UserExportFree;
...
class CourseAccStatisticsExpireController extends Controller
{
    public function course_statistics_expire($course_id){
        if (session()->has('password_notification')) {
            
             return view('dashboard_two.course_statistics_expire.index',compact('course_id'));
         } else {
    ...
    // public function get_all_course_statistics_expire(Request $request)
    // {
    //     if ($request->ajax()) {
    //         // 0 لم يتم النشر
    //         // 1 جاري النشر
    ...
    //                 return view('course_statistics_expire.btn.action', compact('data'));
    //             })
    ...
    public function get_all_users_courses_expire(Request $request,$course_id)
    {
        if ($request->ajax()) {
            // 0 لم يتم النشر
            // 1 جاري النشر
            // 2 تم النشر
            $user_ids= UserCourse::where('course_id',$course_id)->onlyTrashed()->pluck('user_id');
        
            $data = User::join('verified_data', function ($join) use ($user_ids) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('data_subscripe', function ($data) use ($course_id) {
                   $user_course = UserCourse::where('user_id',$data->id)->where('course_id',$course_id)->onlyTrashed()->first();
                   if($user_course){
                ...
                ->addColumn('deleted_at', function ($data) use ($course_id) {
                    $user_course = UserCourse::where('user_id',$data->id)->where('course_id',$course_id)->onlyTrashed()->first();
                    if($user_course){
                        return $user_course->deleted_at;
                    }else{
                        return "-";
                    }
                })
    ...
    public function export_excel($course_id){
        ...
        return Excel::download(new UserExport($coruse->id), $name.'(مدفوع) .xlsx'); // استبدل YourExport بصيغة التصدير الخاصة بك
    }


    public function export_excel_free($course_id){
        ...
        return Excel::download(new UserExportFree($coruse->id), $name.'(مجاني) .xlsx'); // استبدل YourExport بصيغة التصدير الخاصة بك
    }
