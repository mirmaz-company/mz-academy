...
use App\Exports\UserExportExpire;
use App\Models\NotificationTeacher;
use App\Http\Controllers\Controller;
use Maatwebsite\Excel\Facades\Excel;
use App\Exports\UserExportFreeExpire;
...
class CourseAccStatisticsController extends Controller
{
    public function course_statistics($course_id){
        if (session()->has('password_notification')) {
             ...
             return view('dashboard_two.course_statistics.index',compact('course_id'));
         } else {
    ...
    public function export_excel_expire($course_id){
        ...
        return Excel::download(new UserExportExpire($coruse->id), $name.'(مدفوع) .xlsx'); // استبدل YourExport بصيغة التصدير الخاصة بك
    }
    ...
    public function export_excel_free_expire($course_id){
        ...
        return Excel::download(new UserExportFreeExpire($coruse->id), $name.'(مجاني) .xlsx'); // استبدل YourExport بصيغة التصدير الخاصة بك
    }
