...
@section('content')
 ...
 <button class="btn btn-outline-primary" style="display: none" onclick="msg_edit33()" id="position-top-start55"></button> 
...
@section('js')
    ...
    <script>
     ...
     <script type="text/javascript">
        ...
        var table = $('.yajra-datatable').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('get_all_lessons') }}",
            columns: [
                {data: 'DT_RowIndex',  name: 'DT_RowIndex', orderable: false, searchable: false},
                {data: 'name'         ,name: 'name'},
                {data: 'course_id'         ,name: 'course_id'},
                {data: 'teacher_id'         ,name: 'teacher_id'},
                {data: 'type_video'         ,name: 'teacher_id'},
    ...
    <script>
        function publish_now(id){
            var csrfToken = "{{ csrf_token() }}";

            $('#loading6'+ id).css('display','block');
            $('#pu'+ id).css('display','none');


            $.ajax({
                type: 'post',
                url: "{{route('publish_now')}}",
                data: {
                    _token: csrfToken,
                    id: id
                },
