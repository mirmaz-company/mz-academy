...
{
    public function courses_acc(Request $request,$teacher_id){
        ...
        if (session()->has('password_notification')) {
            ...
            return view('dashboard_two.courses_acc.index',compact('teacher_id'));
        } else {
    ...
    public function get_all_courses_acc(Request $request,$teacher_id)
    {
        if ($request->ajax()) {
            $data = CourseTwo::where('type','paid_public')->where('teacher_id',$teacher_id)->orderBy('id','desc');
            return Datatables::of($data)
                ...
                ->addColumn('statistices_expire', function ($data) {
                   return '<a href="'.route('course_statistics_expire',$data->id).'">كشف الاحصائيات</a>';
                })
    
              
                ->rawColumns(['image','statistices','statistices_expire'])
